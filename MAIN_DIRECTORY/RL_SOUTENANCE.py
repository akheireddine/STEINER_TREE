...
#        for nbG in range(nombreDeGenerations):
    ...
    def algorithme_genetique_elitiste(self, N, heuristic, probaMutation, probaCroisement, L, CoupleInstanceHeuristic, event):
    ...
    def local_search(self, heuristic, probaMinRandomisaton, probaMaxRandomisation, L, CoupleInstanceHeuristic, event=Event()):
        debut = time.time()
        dic_heuristic = heuristic()
        I = Graphe_Individu(self, dic_heuristic)
        self.ListeIndividus.append(I)
        meilleur_fitness = self.ListeIndividus[self.calculer_meilleur_fitness()].get_fitness()
        # L.append(meilleur_fitness)       #CORRECTION
        instance, nom_heuristic, opt = CoupleInstanceHeuristic
        print "verif " + instance + " " + str(meilleur_fitness)
        if meilleur_fitness == opt:
            duree = (time.time() - debut)/60.
            ##f.write(instance + " " + str(meilleur_fitness) + " " + str(duree) + "\n")
            # print instance, meilleur_fitness, duree, "au premier coup"
            BASE.append((instance, opt, nom_heuristic, L))
            return
        ...
        while True:
            ...
            if Voisin != None:
                L.append(Voisin.get_fitness())
                self.ListeIndividus.append(Voisin)
                # print "nouveau ameliorant"
            else:
...
#___________A MODIFIER__________
filename = "../B/b02.stp"
heuristic = "PCC"
Opt = 0
timeout = 1
...
instance = "SOUT"
couple_instance_heuristic_opt = instance, heuristic, Opt
RES = list()
t1 = time.time()

if heuristic == "ACM":
    WG.local_search(WG.heuristique_ACPM, probaMinRandomisaton, probaMaxRandomisation, RES, couple_instance_heuristic_opt)
elif heuristic == "PCC":
    WG.local_search(WG.heuristique_PCM, probaMinRandomisaton, probaMaxRandomisation, RES, couple_instance_heuristic_opt)
else:
    print "Choisir heuristic"
t2 = time.time()

print "Recherche Locale ", min(RES), "  temps : ",t2-t1
